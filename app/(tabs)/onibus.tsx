...
import type { SPLine, SPNearbyLine, SPVehicle, SPLinePosition, OSMStop } from '@/constants/sptransTypes';
import {
...
import type { SPArrivalLine } from '@/constants/sptransTypes';
...
export default function OnibusScreen() {
  ...
  const [routeShapes, setRouteShapes]         = useState<{ 1: [number,number][] | null; 2: [number,number][] | null }>({ 1: null, 2: null });
  const [sentido, setSentido]                 = useState<1 | 2>(1);
  const [directionCls, setDirectionCls]       = useState<{ 1: number | null; 2: number | null }>({ 1: null, 2: null });
  ...
  const [centerOn, setCenterOn] = useState<{ lat: number; lon: number; zoom?: number } | null>(null);
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (_, gs) => {
        ...
        sheetHeight.setValue(
          Math.max(SHEET_COLLAPSED, Math.min(SHEET_EXPANDED, base - gs.dy))
        );
      },
      onPanResponderRelease: (_, gs) => {
        ...
        Animated.spring(sheetHeight, {
          ...
          useNativeDriver: false, bounciness: 3,
        }).start();
      },
    })
  ).current;
  ...
  // Centraliza no usuário ao tocar no botão de localização
  ...
  const searchNearby = useCallback(async () => {
    ...
    try {
      ...
      if (lines.length > 0) {
        ...
        Alert.alert('Sem linhas', 'Nenhum ônibus encontrado a 500 m. Tente novamente em instantes.');
      }
    } catch (e) {
  ...
  // ── Select line (from nearby or search) ───────────────────────────────────

  const selectLine = useCallback(async (line: SPNearbyLine | SPLine) => {
    const displayCode = 'c' in line
      ? (line as SPNearbyLine).c
      : `${(line as SPLine).lt}-${(line as SPLine).tl}`;
    Keyboard.dismiss();
    setSelectedLine(line);
    setLineResults([]);
    setSearchQuery('');
    setSheetMode('line');
    // fecha o bottom sheet animado — line mode usa painel fixo
    ...
    // Busca ambos os sentidos em paralelo (GTFS local → GeoServer)
    ...
    const shapes = { 1: shape1, 2: shape2 } as { 1: [number,number][] | null; 2: [number,number][] | null };
    setRouteShapes(shapes);
    setSentido(initialSentido);
    const activeShape = shapes[initialSentido];
    setRouteAvailable(activeShape ? true : null);

    // Descobre cl das duas direções via Linha/Buscar
    ...
    // poll every 10s with direction-specific cl
  ...
  const switchSentido = useCallback((s: 1 | 2) => {
    setSentido(s);
    setRouteStops([]);
    ...
    // Atualiza veículos para o novo sentido
    getPosicaoLinha(cl)
      .then((pos) => {
        const vs: SPVehicle[] = (pos.l ?? []).flatMap((l: SPLinePosition) => l.vs ?? []);
        setVehicles(vs.length > 0 ? vs : (pos.vs ?? []));
    ...
    // Reinicia polling com cl do sentido selecionado
    if (positionInterval.current) clearInterval(positionInterval.current);
    positionInterval.current = setInterval(async () => {
      try {
  ...
  useEffect(() => {
    ...
    searchDebounce.current = setTimeout(async () => {
      try {
        ...
        // Se não encontrou e tem espaço, busca cada termo separado e combina
        if (results.length === 0 && q.includes(' ')) {
          const terms = q.split(/\s+/).filter(t => t.length >= 2);
          const searches = await Promise.all(terms.map(t => buscarLinhas(t).catch(() => [] as SPLine[])));
          const seen = new Set<number>();
          results = [];
          for (const batch of searches) {
            for (const l of batch) {
              if (!seen.has(l.cl)) { seen.add(l.cl); results.push(l); }
            }
          }
        }
  ...
  const selectOsmStop = useCallback(async (stop: OSMStop) => {
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      <View style={styles.mapWrapper}>
        ...
        <View style={styles.searchBar}>
          ...
          {searchQuery.length > 0 && (
            <Pressable style={styles.searchClear} onPress={() => { setSearchQuery(''); setLineResults([]); }} hitSlop={8}>
              <Text style={styles.searchClearText}>✕</Text>
            </Pressable>
          )}
        </View>

        {/* Search dropdown — abre com loader assim que >= 2 chars */}
      ...
      {/* ── Bottom Sheet — apenas nearby-lines e osm-stop ─────────── */}
      <Animated.View style={[styles.sheet, { height: sheetHeight }]} pointerEvents={sheetMode === 'line' ? 'none' : 'auto'}>
          ...
          <ScrollView style={styles.sheetScroll} showsVerticalScrollIndicator={false}>
            ...
            {userCoords && (
              <Text style={styles.sheetSub}>
                {Math.round(haversineMeters(userCoords.lat, userCoords.lon, selectedOsmStop.lat, selectedOsmStop.lon))} m de você
              </Text>
            )}
      ...
      {/* Painel fixo da linha selecionada */}
        ...
        <View style={styles.linePanel}>
          <View style={styles.linePanelHeader}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.linePanelName} numberOfLines={1}>{lineDest}</Text>
            </View>
            {lineLoading
              ? <ActivityIndicator size="small" color={theme.accent} />
              : <Pressable onPress={dismissSheet} hitSlop={16}>
          ...
          <View style={styles.sentidoRow}>
            ...
            >
              <Text style={[styles.sentidoBtnText, sentido === 1 && styles.sentidoBtnTextActive]}>Ida</Text>
              <Text style={[styles.sentidoDest, sentido === 1 && styles.sentidoDestActive]} numberOfLines={1}>
                {selectedLine.lt0}
              </Text>
            </Pressable>
            <Pressable
            ...
            >
              <Text style={[styles.sentidoBtnText, sentido === 2 && styles.sentidoBtnTextActive]}>Volta</Text>
              <Text style={[styles.sentidoDest, sentido === 2 && styles.sentidoDestActive]} numberOfLines={1}>
                {selectedLine.lt1}
              </Text>
            </Pressable>
          </View>
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    ...
    paddingHorizontal: 14, paddingRight: 38, paddingVertical: 10,
  ...
  dropdownDirBtn: {
    flex: 1, borderRadius: 6, paddingVertical: 6, paddingHorizontal: 8,
  ...
  nearbyBtn: {
    ...
    borderWidth: 1, borderColor: theme.border,
    borderRadius: 23, paddingHorizontal: 16, paddingVertical: 12,
  },
  ...
  sectionLabel: {
    color: theme.textFaint, fontSize: 10, letterSpacing: 1.5,
    fontWeight: '600', marginTop: 14, marginBottom: 6,
  },

  // Nearby lines grouped list
  lineGroup: {
    paddingVertical: 12,
    borderBottomWidth: 1, borderBottomColor: theme.border,
  },
  ...
  sentidoBtn: {
    flex: 1, borderRadius: 8, paddingVertical: 8, paddingHorizontal: 10,
    backgroundColor: theme.surfaceElev,
    borderWidth: 1, borderColor: theme.border,
    alignItems: 'center',
  },
  ...
  sentidoBtnText: {
    color: theme.textDim, fontSize: 11, fontWeight: '700', letterSpacing: 0.5,
  },
  sentidoBtnTextActive: { color: theme.onAccent },
  sentidoDest: {
    color: theme.textFaint, fontSize: 10, marginTop: 2,
  },
  ...
  arrivalBadge: {
    backgroundColor: theme.surfaceElev, borderRadius: 4,
