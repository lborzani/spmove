...
import { IcoLogo, IcoSearch, IcoSettings, IcoRefresh } from '@/components/Icons';
...
export default function HomeScreen() {
  ...
  const { data: lines, isLoading, isError, refetch, isRefetching } = useQuery({
    queryKey: ['status'],
    queryFn: fetchStatus,
  ...
  const overall = counts.parado ? 'parado'
    : counts.lento   ? 'lento'
    : counts.atencao ? 'atencao'
    : 'normal';
  ...
  const bannerHeadline =
    overall === 'normal'  ? 'Tudo na linha' :
    overall === 'atencao' ? 'Atenção'        :
    overall === 'lento'   ? 'Rede lenta'     : 'Rede parada';
      ...
      <View style={[styles.banner, { backgroundColor: isLoading ? theme.surface : bannerBg }]}>
          <View style={styles.bannerTopRow}>
            ...
            <View style={styles.bannerTopRight}>
              <Pressable onPress={() => router.push('/settings' as never)} style={styles.headerBtn}>
                <IcoSettings size={16} color={isLoading ? theme.textDim : bannerText} strokeWidth={2} />
              </Pressable>
            </View>
          </View>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <Pressable onPress={() => refetch()} style={[styles.refreshBtnFull, { opacity: isRefetching ? 0.5 : 1 }]} disabled={isRefetching}>
...
const styles = StyleSheet.create({
  ...
  bannerEyebrow: {
    ...
    fontWeight: '600', marginBottom: 8, opacity: 0.75,
  },
  bannerHeadline: {
    fontSize: 40, fontWeight: '700', lineHeight: 38,
    letterSpacing: -2, textTransform: 'uppercase', marginBottom: 10,
  },
  bannerSub:   { fontSize: 13, fontWeight: '600', opacity: 0.85, marginBottom: 16 },
  bannerLoading: {
    alignItems: 'center', justifyContent: 'center', gap: 12, paddingVertical: 24,
  },
  ...
  filtersContainer: {
    paddingHorizontal: 22, gap: 8, flexDirection: 'row',
  },
  chip: {
    borderWidth: 1.5, paddingHorizontal: 14, paddingVertical: 8,
  ...
  skeletonCard: {
    height: 64, borderRadius: theme.radiusCard,
